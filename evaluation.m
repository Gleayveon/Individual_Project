...
%% Evaluation Start
if num == length(listing)
    RDF_V_eachwindow = zeros(num_Sample+1,1);
    RDF_L1_eachwindow = zeros(num_Sample+1,1);
    RDF_L2_eachwindow = zeros(num_Sample+1,1);
    RDF_L3_eachwindow = zeros(num_Sample+1,1);
    ...
    Factor_peak_valley_sample_V = zeros(num_Sample+1,1);
    Factor_rms_sample_V = zeros(num_Sample+1,1);
    Uripple_mean = zeros(num_Sample+1,1);
    Uripple_variance = zeros(num_Sample+1,1);
    ...
    for docount = 1:num_Sample+1
        if docount == num_Sample+1
            voltage = leftover(:,1);
            current_L1 = leftover(:,2);
            current_L2 = leftover(:,3);
            current_L3 = leftover(:,4);
            num_groups = floor(length(leftover) / group_size);
        else
